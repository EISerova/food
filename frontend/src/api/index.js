class Api {
  ...
  getRecipes({
    ...
    return fetch(
      `/api/collect/?page=${page}&limit=${limit}${author ? `&author=${author}` : ''}${is_favorited ? `&is_favorited=${is_favorited}` : ''}${is_in_shopping_cart ? `&is_in_shopping_cart=${is_in_shopping_cart}` : ''}${tagsString}`,
      {
        method: 'GET',
        headers: {
          ...this._headers,
  ...
  downloadFile() {
    const token = localStorage.getItem('token')
    return fetch(
      `/api/collect/download_shopping_cart/`,
      {
        method: 'GET',
        headers: {
          ...this._headers,
